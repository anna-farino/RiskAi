...
- **User Opt-In Source Model**: Global sources without an explicit entry in `user_source_preferences` are treated as disabled by default. Users must explicitly enable sources they want to see content from by creating a preference record with `isEnabled: true`. This provides user control over content feeds while keeping the database clean and avoiding the need to create preference records for all users whenever a source is added.
