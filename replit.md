...
- **Fact-Based Threat Severity Scoring** (November 2025): Replaced wasteful dual scoring system (~$100/month OpenAI waste) with a single fact-based approach using GPT-4o-mini structured outputs. System extracts objective, verifiable facts (exploitation status, impact details, patch availability, detection difficulty) and applies deterministic rubric-based scoring. Features: (1) Transparent evidence trails with reasoning for each score, (2) Robust 4-strategy baseline fallback (extraction_successful flag, null checks, confidence validation, 4-layer filler phrase detection), (3) Guaranteed baseline scores (exploitability=3, impact=3, patch_status=5, detection_difficulty=5) when extraction fails, (4) No keyword fallback - fact-based only with graceful degradation, (5) extractedFacts stored in database for audit. **Production Validation Fixes** (November 2025): Relaxed validation thresholds from strict test requirements to real-world production needs: (a) Accept partial facts (1 fact minimum vs 3), (b) Lower evidence requirement (5 chars vs 10), (c) Single-category confidence check (OR vs AND), (d) Comprehensive NaN protection (default undefined confidence to 0.6), (e) Save partial facts with warnings instead of discarding. Testing against 10 real database articles shows 80% fact extraction success rate (vs 0% with strict validation), with only appropriate baseline fallbacks for non-vulnerability content. **Confidence System Overhaul v2.1** (November 2025): Eliminated harsh penalties for missing CVEs/threat actors/hardware (most legitimate articles don't have these). New system: (1) Only 2 confidence flags: `low_fact_confidence` (<50% AI confidence) and `no_specific_targets` (no software/hardware/attack vectors/companies), (2) Smarter target detection using `hasSpecificTargets()` - checks for ANY specific target (software > hardware > attack vectors > companies with confidence weighting), (3) Revised penalty tiers: 0 flags = 0%, 1 flag = 10%, 2+ flags = 25% (vs old 30% for 3+ flags), (4) CVE/Threat Actor bonuses: +10% each when present (additive rewards, not penalties for absence), (5) Lowered thresholds: critical 85+, high 60-84, medium 30-59, low 0-29 (vs old 90/70/40/0). Result: Real attacks with specific targets (e.g., "ClickFix targets browsers") now correctly score as "medium" instead of being penalized to "low" for missing CVEs. Net improvement: ClickFix malware improved from 33.95/low to 48.5/medium (+14.55 points), proper severity classification for 80%+ of threat articles. Implementation: `backend/services/threat-analysis.ts` v2.1, `backend/scripts/test-scoring-v2.1.ts`.
