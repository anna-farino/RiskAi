...
- **User Opt-In Source Model**: When admins add or enable a global source, it is automatically added to all users' `user_source_preferences` table with `isEnabled: false`. This allows users to discover new sources and opt-in by manually enabling them in their own source preferences, maintaining user control over their content feeds while enabling centralized source management.
