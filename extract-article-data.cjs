...
async function extractArticleData(url) {
  try {
    ...
    // Extract the headline - prioritize meta tags for more reliability
    ...
    if (!headline) {
      // Avoid getting h1 elements that are part of navigation or sidebars
    ...
    // If still no headline, use the page title (but clean it up)
    if (!headline) {
