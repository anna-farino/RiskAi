...
export function ArticleCard({ article, onDelete, isPending = false, onKeywordClick, onSendToCapsule, isSending = false }: ArticleCardProps) {
  ...
  const handleSendToCapsule = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (onSendToCapsule) {
      onSendToCapsule(article.url);
    }
  };
          ...
          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
            ...
            {article.publishDate && (
              <div className="flex items-center gap-1 sm:gap-1.5 text-xs text-slate-400">
                ...
                <span className="text-[10px] sm:text-xs">{formatDistanceToNow(new Date(article.publishDate))} ago</span>
              </div>
            )}
          </div>
              ...
              {onSendToCapsule && (
                <Button
                  ...
                  className={cn(
                    ...
                    (isPending || isSending) && "cursor-not-allowed opacity-70"
                  )}
