...
export default function LiveLogs() {
  ...
  const runArticleTestScraping = async () => {
    if (!testUrl.trim()) {
      toast({
        title: "Article URL Required",
        description: "Please enter a valid article URL to test.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      const response = await fetch(`${serverUrl}/api/test-scraping/test-article`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (data.success) {
        setTestResult({
          success: true,
          message: `Article test initiated! Processing: ${testUrl}`
        });
        toast({
          title: "Article Test Started",
          description: "Article scraping test initiated. Check the logs tab for detailed results.",
        });
      } else {
        setTestResult({
          success: false,
          message: data.error || 'Article test failed'
        });
        toast({
          title: "Article Test Failed",
          description: data.error || 'Article scraping test failed',
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Article Test Error",
        description: "Failed to run article scraping test. Check your connection.",
        variant: "destructive"
      });
    } finally {
      setTestRunning(false);
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="logs" className="w-full">
        ...
        <TabsContent value="test" className="space-y-4 mt-6">
          ...
          {/* Test Single Article Panel */}
          <Card className="bg-black/40 backdrop-blur border border-[#00FFFF]/20">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="article-url" className="text-white">Article URL</Label>
                <Input
                  id="article-url"
                  type="url"
                  placeholder="https://example.com/article/cybersecurity-threat"
                  value={testUrl}
                  onChange={(e) => setTestUrl(e.target.value)}
                  disabled={isTestRunning}
                  className="bg-black/60 border-2 border-[#00FFFF]/30 text-white placeholder:text-gray-400 focus:border-[#BF00FF]"
                />
              </div>

              <Button
                onClick={runArticleTestScraping}
                disabled={!testUrl.trim() || isTestRunning}
              ...
              <div className="text-xs text-gray-400 space-y-1">
                <p>• Tests article scraping through the normal production pipeline</p>
                <p>• Extracts content, AI analysis, and entity detection</p>
                <p>• Saves article to database if successful</p>
                <p>• Check logs tab for detailed extraction results</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
