...
export default function Reports() {
  ...
  const loadExecutiveNotes = async (reportId: string) => {
    try {
      const response = await fetch(
        `/api/news-capsule/executive-notes/${reportId}`,
        {
          credentials: "include",
        },
      );
  ...
  const removeArticleFromReport = (articleId: string) => {
    ...
    const updatedReport = {
      ...selectedReport,
      articles: selectedReport.articles.filter(
    ...
    const savedReports = JSON.parse(
      localStorage.getItem("newsCapsuleReports") || "[]",
    );
                      ...
                      if (savedReportsStr) {
                        ...
                        const reportsFromSameDay = savedReports.filter(
                          (report) => {
                            ...
                            return (
                              reportDay.getTime() === selectedDate.getTime()
                            );
                          },
                        );
                        ...
                        if (reportsFromSameDay.length > 1) {
                          ...
                          selectDialog.style.backgroundColor =
                            "rgba(0, 0, 0, 0.8)";
                          ...
                          const sortedReports = [...reportsFromSameDay].sort(
                            (a, b) =>
                          ...
                          exportButton.addEventListener("click", () => {
                            ...
                            if (reportToExport) {
                              ...
                              const versionText =
                                reportToExport.versionNumber &&
                                reportToExport.versionNumber > 1
              ...
              {selectedReport.articles.length === 0 ? (
                <div className="p-4 bg-blue-900/30 border border-blue-700/30 rounded-lg mb-6">
                  <h3 className="text-blue-300 font-medium mb-2">
                    Empty Report Created Successfully
                  </h3>
                  <p className="text-sm text-slate-300 mb-2">
                    This is a new empty report that doesn't contain any articles
                    yet.
                  </p>
                  <p className="text-sm text-slate-400">
                    To add articles to this report:
                  </p>
                  <ol className="list-decimal list-inside text-sm text-slate-400 ml-2 mt-1 space-y-1">
                    ...
                    <li>
                      Choose "Send to Executive Report" and select this report
