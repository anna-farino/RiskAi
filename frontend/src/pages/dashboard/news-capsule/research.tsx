...
export default function Research() {
  ...
  useEffect(() => {
    if (processedArticles && Array.isArray(processedArticles)) {
      ...
      // Update selected articles based on markedForReporting
      const selected = uniqueArticles.filter((article: any) => article.markedForReporting);
  ...
  const addUrlMutation = useMutation({
    mutationFn: (urlData: { url: string }) => 
      apiRequest('POST', '/api/news-capsule/process-url', urlData),
    onSuccess: () => {
      refetch();
  ...
  const markForReportingMutation = useMutation({
    mutationFn: ({ articleId, markedForReporting }: { articleId: string, markedForReporting: boolean }) =>
      apiRequest('POST', '/api/news-capsule/add-to-report', { articleId, markedForReporting }),
    onSuccess: () => {
      refetch();
    },
    onError: (error) => {
  ...
  const deleteArticleMutation = useMutation({
    mutationFn: (articleId: string) =>
      apiRequest('POST', '/api/news-capsule/add-to-report', { articleId, markedForDeletion: true }),
    onSuccess: () => {
      refetch();
    },
    onError: (error) => {
