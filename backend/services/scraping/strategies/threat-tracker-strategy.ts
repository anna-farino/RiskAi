    ...
    return {
      ...
      aiProviders: {
        identifyArticleLinks: async (html: string, url: string) => {
          ...
          const { identifyArticleLinksWithAI } = await import('backend/apps/threat-tracker/services/openai');
          return identifyArticleLinksWithAI(html, url);
        },
        ...
        extractPublishDate: async (html: string, url: string, selectors?: any) => {
          const { extractDateFromHtml } = await import('backend/apps/threat-tracker/services/content-extraction');
          return extractDateFromHtml(html, url, selectors);
        }
      },
      extractionOptions: {
