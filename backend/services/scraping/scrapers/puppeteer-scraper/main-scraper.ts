 ...
 */
async function warmSessionBeforeTarget(
  page: Page,
  browserProfile: any,
): Promise<void> {
  try {
    log(
      `[PuppeteerScraper] Warming session with neutral sites before target...`,
      "scraper",
    );
    ...
    const sitesToVisit = neutralSites
    ...
    for (const site of sitesToVisit) {
      try {
        ...
        log(
          `[PuppeteerScraper] Failed to warm with ${site}: ${e.message}`,
          "scraper",
        );
      }
    }
    ...
    log(
      `[PuppeteerScraper] Session warming completed, proceeding to target`,
      "scraper",
    );
  } catch (error: any) {
    log(
      `[PuppeteerScraper] Session warming error: ${error.message}`,
      "scraper",
    );
...
async function waitForPreloaderComplete(page: Page): Promise<void> {
  ...
  try {
    ...
    await page
      ...
      .catch(() => {
        log(
          `[PuppeteerScraper] Preloader element wait timeout, continuing...`,
          "scraper",
        );
      });
    ...
    await page
      ...
      .catch(() => {
        log(
          `[PuppeteerScraper] Content appearance wait timeout, continuing...`,
          "scraper",
        );
      });
    ...
    log(
      `[PuppeteerScraper] Preloader wait completed in ${totalWaitTime}ms`,
      "scraper",
    );
  } catch (error: any) {
...
export async function scrapeWithPuppeteer(
  ...
  try {
    log(
      `[PuppeteerScraper] Starting enhanced Puppeteer scraping for: ${url}`,
      "scraper",
    );
    ...
    log(
      `[PuppeteerScraper] Using ${browserProfile.deviceType} browser profile`,
      "scraper",
    );
    ...
    if (options?.isArticlePage) {
      page = await setupArticlePage({
        ...
        timeouts: {
          navigation: options.timeout || 60000,
      ...
      page = await setupSourcePage({
        headers: options.customHeaders || browserProfile.headers,
        timeouts: {
          navigation: options.timeout || 45000,
          default: options.timeout || 45000,
        },
      });
    }
    ...
    log(
      `[PuppeteerScraper] Page setup completed, navigating to URL`,
      "scraper",
    );
    ...
    try {
      ...
      log(
        `[PuppeteerScraper] Navigating with ${waitCondition} (timeout: ${navTimeout}ms)`,
        "scraper",
      );
      ...
      try {
        ...
        if (
          networkIdleError.message?.includes("timeout") ||
          networkIdleError.message?.includes("Navigation timeout") ||
          networkIdleError.message?.includes("waiting for event")
        ) {
          log(
            `[PuppeteerScraper] networkidle2 failed (${networkIdleError.message.substring(0, 100)}), falling back to domcontentloaded`,
            "scraper",
          );
          ...
          log(
            `[PuppeteerScraper] Using domcontentloaded fallback, waiting for content...`,
            "scraper",
          );
      ...
      if (
        ...
        log(
          `[PuppeteerScraper] Frame detached during navigation - attempting recovery`,
          "scraper",
        );
        ...
        try {
          ...
          if (html && html.length > 1000) {
            log(
              `[PuppeteerScraper] Retrieved content via page.content() despite frame detachment`,
              "scraper",
            );
            return {
              html,
          ...
          const evaluatedHtml = await page
            .evaluate(() => document.documentElement.outerHTML)
            .catch(() => "");
          if (evaluatedHtml && evaluatedHtml.length > 1000) {
            log(
              `[PuppeteerScraper] Retrieved content via evaluate despite frame detachment`,
              "scraper",
            );
            return {
              html: evaluatedHtml,
          ...
          log(
            `[PuppeteerScraper] Creating new page for retry with stealth navigation`,
            "scraper",
          );
          ...
          if (retryResponse || page.url() !== "about:blank") {
            ...
            if (retryHtml && retryHtml.length > 500) {
              log(
                `[PuppeteerScraper] Successfully retrieved content on retry`,
                "scraper",
              );
              return {
                html: retryHtml,
          ...
          log(
            `[PuppeteerScraper] Recovery attempt failed: ${recoveryError.message}`,
            "scraper",
          );
        }
      }
    ...
    log(
      `[PuppeteerScraper] Navigation completed. Status: ${statusCode}`,
      "scraper",
    );
    ...
    if (response && !response.ok()) {
      log(
        `[PuppeteerScraper] Warning: Response status is not OK: ${statusCode}`,
        "scraper",
      );

      if (statusCode === 401) {
        log(
          `[PuppeteerScraper] 401 Unauthorized - attempting paywall bypass strategies`,
          "scraper",
        );
        ...
        try {
          ...
          log(
            `[PuppeteerScraper] Retrying with Googlebot user agent for paywall bypass`,
            "scraper",
          );
          ...
          if (retryLinks >= 10) {
            log(
              `[PuppeteerScraper] Paywall bypass successful with Googlebot (${retryLinks} links)`,
              "scraper",
            );
            ...
            log(
              `[PuppeteerScraper] Googlebot bypass insufficient (${retryLinks} links), continuing with original content`,
              "scraper",
            );
          }
        } catch (bypassError: any) {
          log(
            `[PuppeteerScraper] Paywall bypass attempt failed: ${bypassError.message}`,
            "scraper",
          );
        }
      } else if (statusCode === 403) {
        log(
          `[PuppeteerScraper] 403 Forbidden - site blocking access`,
          "scraper",
        );
      } else if (statusCode === 503) {
        log(
          `[PuppeteerScraper] 503 Service Unavailable - site temporarily down or overloaded`,
          "scraper",
        );
      }
    }
    ...
    if (options?.protectionBypass !== false) {
      ...
      if (isDataDomeChallenge) {
        log(
          `[PuppeteerScraper] DataDome challenge detected, attempting enhanced bypass`,
          "scraper",
        );
        ...
        if (protectionCheck.hasProtection) {
          log(
            `[PuppeteerScraper] ${protectionCheck.type} protection detected, attempting bypass`,
            "scraper",
          );
          ...
          if (!bypassSuccess) {
            log(
              `[PuppeteerScraper] ${protectionCheck.type} protection bypass failed`,
              "scraper",
            );
          } else {
            log(
              `[PuppeteerScraper] ${protectionCheck.type} protection bypass successful`,
              "scraper",
            );
            await performBehavioralDelay(options);
          }
    ...
    log(
      `[PuppeteerScraper] Initial content extracted (${contentLength} chars)`,
      "scraper",
    );
    ...
    const validation = await validateContent(
      html,
      url,
      options?.isArticlePage || false,
    );

    if (!validation.isValid || validation.isErrorPage) {
      log(
        `[PuppeteerScraper] Content validation warning: ${validation.errorIndicators.join(", ")}, confidence: ${validation.confidence}%`,
        "scraper",
      );
    }
    ...
    if (!options?.isArticlePage && validation.linkCount < 10) {
      log(
        `[PuppeteerScraper] Warning: Only ${validation.linkCount} links found (minimum 10 recommended for source pages)`,
        "scraper",
      );
    }
    ...
    const htmxDetected = await page.evaluate(() => {
      return (
        ...
        !!document.querySelector(
          '[hx-get], [hx-post], [hx-trigger], script[src*="htmx"]',
        ) ||
        !!document.querySelector(
    ...
    const preloaderDetected = await page
      .evaluate(() => {
        try {
          ...
          const bodyText = document.body?.textContent || "";
          const hasPreloaderText = /loading|please wait|getting ready/i.test(
            bodyText,
          );

          // Check for common preloader JavaScript patterns
          const hasPreloaderJS =
    ...
    const needsDynamicLoading =
      hasMinimalContent || htmxDetected || preloaderDetected;

    if (needsDynamicLoading) {
      log(
        `[PuppeteerScraper] Dynamic loading needed - minimal content: ${hasMinimalContent} (${contentLength} chars), HTMX detected: ${htmxDetected}, preloader detected: ${preloaderDetected}`,
        "scraper",
      );
      ...
      if (options?.handleHTMX && htmxDetected) {
        log(
          `[PuppeteerScraper] HTMX detected on page, loading HTMX content`,
          "scraper",
        );
        ...
        if (htmxAssessment && htmxAssessment.links) {
          ...
          log(
            `[PuppeteerScraper] HTMX extraction captured ${htmxLinks.length} links (complexity: ${htmxAssessment.htmxComplexity}, needsAdvanced: ${htmxAssessment.needsAdvancedExtraction})`,
            "scraper",
          );
        }
      }
      ...
      if (preloaderDetected) {
        log(
          `[PuppeteerScraper] Preloader detected, waiting for content to load...`,
          "scraper",
        );
      ...
      log(
        `[PuppeteerScraper] Dynamic content loading completed (${dynamicContentLength} chars)`,
        "scraper",
      );

      // Validate dynamic content - use isArticlePage option to determine validation type
      const dynamicValidation = await validateContent(
        dynamicHtml,
        url,
        options?.isArticlePage || false,
      );
      ...
      const shouldUseDynamic =
        ...
        (dynamicContentLength > contentLength * 1.2 &&
          dynamicValidation.linkCount >= validation.linkCount);

      if (shouldUseDynamic) {
        log(
          `[PuppeteerScraper] Using dynamic content (${dynamicValidation.linkCount} links, ${dynamicValidation.confidence}% confidence, HTMX: ${htmxDetected})`,
          "scraper",
        );
        ...
        const isSuccess =
          ...
          !dynamicValidation.isErrorPage &&
          (options?.isArticlePage || dynamicValidation.linkCount >= 10);

        if (statusCode === 401) {
          log(
            `[PuppeteerScraper] Paywall detected (${dynamicValidation.linkCount} links) - may need enhanced bypass`,
            "scraper",
          );
        }
        ...
        log(
          `[PuppeteerScraper] Dynamic content did not provide improvement (original: ${validation.linkCount} links, dynamic: ${dynamicValidation.linkCount} links)`,
          "scraper",
        );
      }
    } else {
      log(
        `[PuppeteerScraper] Substantial content already extracted (${contentLength} chars), skipping dynamic loading`,
        "scraper",
      );
    }

    log(
      `[PuppeteerScraper] Content extraction completed successfully (${validation.linkCount} links, ${validation.confidence}% confidence)`,
      "scraper",
    );
    ...
    if (statusCode === 401) {
      log(
        `[PuppeteerScraper] Paywall detected (${validation.linkCount} links) - source page access blocked`,
        "scraper",
      );
    }
    ...
    if (isExternalValidationError(error)) {
      log(
        `[PuppeteerScraper] External validation warning in main function (continuing): ${error.message}`,
        "scraper",
      );
    ...
    if (isBrowserError) {
      log(
        `[PuppeteerScraper] Browser disconnection detected: ${error.message}`,
        "scraper-error",
      );
      ...
      try {
        log(
          `[PuppeteerScraper] Attempting to restart browser for recovery`,
          "scraper",
        );
        ...
        log(
          `[PuppeteerScraper] Failed to restart browser: ${restartError.message}`,
          "scraper-error",
        );
      }
    }
    ...
    log(
      `[PuppeteerScraper] Error during Puppeteer scraping: ${error.message}`,
      "scraper-error",
    );
    ...
    if (page) {
      try {
        ...
        log(
          `[PuppeteerScraper] Error closing page: ${closeError.message}`,
          "scraper-error",
        );
      }
    }
 ...
 */
export async function scrapeWithStealthPuppeteer(
  url: string,
  options?: PuppeteerScrapingOptions,
): Promise<ScrapingResult> {
