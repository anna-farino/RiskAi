    ...
    if (existingLinkData && existingLinkData.length > 0 && !shouldForceExtraction) {
      ...
      if (hasHtmx.scriptLoaded || hasHtmx.htmxInWindow || hasHtmx.hasHxAttributes) {
        ...
        if (externalArticleUrls.length > 0) {
          ...
          // Step 3: For Foorilla-style aggregator URLs, extract the original source URLs
          const foorillaUrls = articleLinkData.filter(item => 
            item.href && item.href.includes('foorilla.com/media/items/')
          );
          
          if (foorillaUrls.length > 0) {
            log(`[LinkExtractor] Step 3: Found ${foorillaUrls.length} Foorilla aggregator URLs, extracting original source URLs...`, "scraper");
            
            const originalSourceUrls = [];
            
            // Process up to 15 Foorilla URLs to avoid excessive requests
            for (const foorillaItem of foorillaUrls.slice(0, 15)) {
              try {
                log(`[LinkExtractor] Extracting source URL from: ${foorillaItem.href}`, "scraper");
                
                // Navigate to the Foorilla page and extract the original source URL
                const sourceUrl = await page.evaluate(async (foorillaUrl) => {
                  try {
                    // Fetch the Foorilla page content
                    const response = await fetch(foorillaUrl);
                    ...
                    // Look for the original source link in common patterns
                    const sourceSelectors = [
                      'a[href*="siliconangle.com"]',
                      ...
                      'a[href*="wired.com"]',
                      'a[href*="engadget.com"]',
                      'a[href*="thehackernews.com"]',
                      'a[href*="bleepingcomputer.com"]',
                      'a[href*="securityweek.com"]',
                      'a[href*="darkreading.com"]',
                      'a[href*="cybersecuritynews.com"]',
                      'a[rel="nofollow"]',
                      'a[target="_blank"]',
                      '.source-link a',
                      '.article-source a',
                    ...
                    for (const selector of sourceSelectors) {
                      const sourceLink = tempDiv.querySelector(selector);
                      if (sourceLink) {
                        const href = sourceLink.getAttribute('href');
                        if (href && href.startsWith('http') && !href.includes('foorilla.com')) {
                          console.log(`Found source URL: ${href}`);
                          return href;
                        }
                      }
                    }
                    
                    // Fallback: Look for any external links that aren't social media or ads
                    const allExternalLinks = tempDiv.querySelectorAll('a[href^="http"]');
                    for (const link of allExternalLinks) {
                      const href = link.getAttribute('href');
                      if (href && !href.includes('foorilla.com') && 
                          ...
                          !href.includes('amazon.com') && !href.includes('affiliate') &&
                          (href.includes('news') || href.includes('tech') || href.includes('cyber') ||
                           href.includes('security') || href.includes('article') || href.includes('blog'))) {
                        console.log(`Found potential source URL: ${href}`);
                        return href;
                      }
                    }
                    ...
                    console.error(`Error extracting source from ${foorillaUrl}:`, error);
                    return null;
                  }
                }, foorillaItem.href);
                
                if (sourceUrl) {
                  log(`[LinkExtractor] Successfully extracted source URL: ${sourceUrl}`, "scraper");
                  originalSourceUrls.push({
                    href: sourceUrl,
                    text: foorillaItem.text,
                    context: `Original source extracted from Foorilla`,
                    parentClass: 'foorilla-extracted-source'
                  });
                } else {
                  log(`[LinkExtractor] Could not extract source URL from: ${foorillaItem.href}`, "scraper");
                  // Keep the original Foorilla URL as fallback
                  originalSourceUrls.push(foorillaItem);
                }
                ...
                log(`[LinkExtractor] Error processing Foorilla URL ${foorillaItem.href}: ${error.message}`, "scraper-error");
                // Keep the original Foorilla URL as fallback
                originalSourceUrls.push(foorillaItem);
              }
            }
            
            // Replace Foorilla URLs with extracted source URLs
            const nonFoorillaUrls = articleLinkData.filter(item => 
              !item.href || !item.href.includes('foorilla.com/media/items/')
            );
            
            articleLinkData = [...nonFoorillaUrls, ...originalSourceUrls];
            
            log(`[LinkExtractor] Step 3 Complete: Extracted ${originalSourceUrls.length} original source URLs from Foorilla pages`, "scraper");
          }
