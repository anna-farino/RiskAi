...
/**
 * Extract article links from Puppeteer page with comprehensive HTMX handling
 * Based on proven working implementation from Threat Tracker
 */
async function extractLinksFromPage(page: Page, baseUrl: string, options?: LinkExtractionOptions, existingLinkData?: LinkData[]): Promise<LinkData[]> {
  try {
    ...
    // Comprehensive HTMX detection (do this regardless of existing link data)
    ...
    if (existingLinkData && existingLinkData.length > 0 && !shouldForceExtraction) {
      ...
      if (hasHtmx.scriptLoaded || hasHtmx.htmxInWindow || hasHtmx.hasHxAttributes) {
        ...
        // Specifically for dynamic sites, manually fetch HTMX content
        ...
        if (hasHtmx.scriptLoaded || hasHtmx.htmxInWindow || hasHtmx.hasHxAttributes) {
          ...
          if (hasHtmx.hxGetElements.length > 0) {
            ...
            if (filteredEndpoints.length > 0) {
              log(`[LinkExtractor] Triggering HTMX requests directly via fetch`, "scraper");
    ...
    log(`[LinkExtractor] Final extraction result: ${articleLinkData.length} links from dynamic page`, "scraper");
...
/**
 * Extract article links from Puppeteer page with comprehensive HTMX handling
 * This is the main export function that should be used by scrapers
 */
export async function extractArticleLinksFromPage(
  ...
  try {
    log(`[LinkExtractor] Starting dynamic link extraction for: ${baseUrl}`, "scraper");
    
    // Extract links using comprehensive HTMX handling
    const linkData = await extractLinksFromPage(page, baseUrl, options, existingLinkData);
    
    if (linkData.length === 0) {
      log(`[LinkExtractor] No links found from dynamic page`, "scraper");
      return [];
    }
    ...
    log(`[LinkExtractor] Final result: ${links.length} article links extracted from dynamic page`, "scraper");
    return links;
    
  } catch (error: any) {
    log(`[LinkExtractor] Error during dynamic link extraction: ${error.message}`, "scraper-error");
