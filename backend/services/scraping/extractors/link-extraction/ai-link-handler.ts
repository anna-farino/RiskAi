...
export async function handleAILinkIdentification(
  ...
  try {
    ...
    // Step 2: Resolve redirects BEFORE OpenAI analysis (Sequential Processing)
    ...
    for (let i = 0; i < normalizedLinks.length; i++) {
      const link = normalizedLinks[i];
      
      try {
        log(`[LinkExtractor] Processing URL ${i + 1}/${normalizedLinks.length}: ${link.href.substring(0, 60)}...`, "scraper");
        ...
        if (redirectResult.isRedirect) {
          ...
          // Puppeteer fallback for any URL that failed HTTP resolution (ONE AT A TIME)
