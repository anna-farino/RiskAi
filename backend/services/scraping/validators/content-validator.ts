...
export function isCorruptedText(text: string): boolean {
  ...
  if (nonAsciiRatio > 0.7) {
    ...
    log(`[ContentValidator] Sample of text with high non-ASCII: "${text.slice(0, 200).replace(/\s+/g, ' ')}"`, "scraper");
    return true;
  }
  ...
  if (wordRatio < 0.3 && text.length > 100) {
    ...
    log(`[ContentValidator] Sample of text with low word ratio: "${text.slice(0, 200).replace(/\s+/g, ' ')}"`, "scraper");
    return true;
  }
  ...
  if (gibberishMatch) {
    ...
    const isLegitimate = legitimatePatterns.some(pattern => pattern.test(matchSample));
    
    if (!isLegitimate) {
      // Log the actual matched pattern and surrounding context
      const matchIndex = text.indexOf(matchSample);
      const contextStart = Math.max(0, matchIndex - 50);
      const contextEnd = Math.min(text.length, matchIndex + matchSample.length + 50);
      const contextSample = text.slice(contextStart, contextEnd);
      
      log(`[ContentValidator] WARNING: Repeated pattern detected (may be false positive): "${matchSample}"`, "scraper");
      log(`[ContentValidator] Context around pattern: "...${contextSample}..."`, "scraper");
      log(`[ContentValidator] First 200 chars of text: "${text.slice(0, 200).replace(/\s+/g, ' ')}"`, "scraper");
      ...
      if (uniquePatterns > 2 || matchSample.length > 50) {
        log(`[ContentValidator] Text marked as corrupted due to ${uniquePatterns} unique repeated patterns or long pattern (${matchSample.length} chars)`, "scraper");
        return true;
      } else {
        log(`[ContentValidator] Repeated pattern found but not marking as corrupted (likely false positive)`, "scraper");
      }
    }
  }
